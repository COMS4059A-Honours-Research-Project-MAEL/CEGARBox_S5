...
# treat unsolved as this time (in ms)
TL_MS = 180_000.0
...
def make_results(name, times_ms, resolutions):
    ...
    for t, r in zip(times_ms, resolutions):
        ...
        if r in SOLVED_STATUSES:
            ...
            solved_mask.append(False)
            # if you want non-solved with a positive time included in averages,
            # change this to t instead of np.nan
            avg_times_ms.append(np.nan)
...
def aggregate_group(group_dirs, solver_key, ref_solver_key="CEGARBox_S5"):
    """
    ...
    for fam_dir in group_dirs:
        ...
        if json_candidates:
            ...
            ref_candidates = list(
                (fam_dir / ref_dir).glob(
                    f"{ref_dir}-{fam_dir.name}-results.json"
                )
            )
            if not ref_candidates:
                print(
                    f"Warning: no JSON for {solver_key} or reference "
                    f"{ref_solver_key} in family {fam_dir.name}; skipping."
                )
                continue
...
def plot_instances_solved(solver_results, time_points_s, title="MCNF Benchmarks"):
...
# -------------------------------------------------------------------
# directory scanning and table output
# -------------------------------------------------------------------

def find_mcnf_groups(results_root: Path):
    """
    Returns:
        groups: dict d -> [Path(MCNF_d_...)]
        all_dirs: list of all MCNF_* dirs
    """
    ...
    for d in results_root.iterdir():
        if not d.is_dir():
            continue
        if not d.name.startswith("MCNF_"):
            continue
    ...
    all_dirs = sorted(all_dirs, key=lambda p: p.name)
    return groups, all_dirs


def print_table_rows(groups):
    """
    Print LaTeX-friendly rows for MCNF(d=2), MCNF(d=4), MCNF(d=6), All MCNF.
    """
    depths = sorted(groups.keys())
    all_dirs = [d for depth in depths for d in groups[depth]]

    def summarize(group_name, dirs):
        print(f"% {group_name}")
        totals = {}
        stats = {}
        for solver_key in SOLVER_ORDER:
            res = aggregate_group(dirs, solver_key)
            total = len(res["times_ms"])
            solved = int(res["solved_mask"].sum())

            # if nothing solved, average = TL_MS
            if solved == 0:
                avg_ms = TL_MS
            else:
        ...
        # use CEGARBox_S5's total as the family size (should be same for all)
        total_family = totals["CEGARBox_S5"]

        # format a LaTeX row
        row = (
            f"{group_name:11s} & {total_family} & "
    ...
    # per depth
    for d in depths:
        summarize(f"MCNF($d={d}$)", groups[d])

    # all MCNF combined
    summarize("All MCNF   ", all_dirs)
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python mcnf_plots.py /path/to/Results")
        sys.exit(1)

    results_root = Path(sys.argv[1]).expanduser().resolve()
    groups, all_dirs = find_mcnf_groups(results_root)

    # 1) print LaTeX rows for the table
    print_table_rows(groups)

    # 2) plots using ALL MCNF instances
    all_results = []
    for solver_key in SOLVER_ORDER:
        res = aggregate_group(all_dirs, solver_key)
